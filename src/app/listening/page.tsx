...
import { LISTENING_PASSAGES } from "@/lib/content/passages";
import { useState } from "react";

export default function ListeningPage() {
  const [pIdx, setPIdx] = useState(0);
  const passage = LISTENING_PASSAGES[pIdx % LISTENING_PASSAGES.length]!;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Compréhension orale</h1>
        <p className="mt-1 text-sm text-[var(--muted)]">
          Script type message vocal — lisez à voix haute ou intégrez TTS plus tard ; questions piégées commentées.
        </p>
      </div>
      ...
      <Card>
        ...
        {passage.audioScriptNote ? (
          <p className="mt-2 text-xs text-[var(--muted)]">{passage.audioScriptNote}</p>
        ) : null}
      </Card>
        ...
        <Card key={q.id}>
          ...
          {choices[q.id] !== undefined ? (
            <div className="mt-3">
              ...
              {showExp[q.id] ? <CardDescription className="mt-2">{q.explanationFr}</CardDescription> : null}
            </div>
          ) : null}
        </Card>
      ))}
